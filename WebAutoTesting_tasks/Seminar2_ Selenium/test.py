"""
Добавить в наш тестовый проект шаг добавления поста после входа. Должна выполняться проверка на наличие названия поста
на странице сразу после его создания.
...
def test_step3(site, log_xpath, pass_xpath, btn_xpath, btn_create_post,
    ...
    input1.send_keys("Hello_world!")
    input2 = site.find_element("xpath", description_post_xpath)
    input2.send_keys("Здравствуй, мир))")
    input2 = site.find_element("xpath", content_post_xpath)
    input2.send_keys(" ".join(random.choices(string.ascii_uppercase + string.digits, k=10)))
